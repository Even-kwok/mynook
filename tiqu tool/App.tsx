...
import { toBase64, cropImage, embedPromptInImage } from './utils/imageUtils';
...
const PhotoDisplay: React.FC<{
    ...
    prompt: string;
    imageUrl: string;
    onDownload: (imageUrl: string, era: string, prompt: string) => void;
    onRegenerate: () => void;
            ...
            <div className="absolute top-4 right-4 z-10" ref={menuRef}>
                ...
                {isMenuOpen && (
                    ...
                    >
                        ...
                        <button onClick={() => { onDownload(imageUrl, era, prompt); setIsMenuOpen(false); }} className="w-full text-left px-3 py-2 hover:bg-blue-500/10 rounded-md transition-colors">Download</button>
                    </motion.div>
                )}
            </div>
        </motion.div>
...
const App: React.FC = () => {
    ...
    const [extractorImages, setExtractorImages] = useState<(string | null)[]>(Array(70).fill(null));
...
// Recipe Version: MyNook-V1.0-Universal`);
    ...
    const handleExtractorImageUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const availableSlots = 70 - currentImageCount;
    
        if (availableSlots <= 0) {
            setError("You have reached the maximum of 70 images.");
            return;
        }
    ...
    const handleExtractPrompts = async () => {
        ...
        const extractionThunks = imagesToProcess.map(img =>
            () => extractPromptFromImage(img.split(',')[1], extractorPrompt)
        );
        
        const results: PromiseSettledResult<string>[] = [];
        for (let i = 0; i < extractionThunks.length; i += CONCURRENCY_LIMIT) {
            const batch = extractionThunks.slice(i, i + CONCURRENCY_LIMIT);
            const batchResults = await Promise.allSettled(batch.map(thunk => thunk()));
        ...
        if (successfulPrompts.length > 0) {
            ...
            setUserPrompts(combinedPrompts.slice(0, 70));
        ...
        setExtractorImages(Array(70).fill(null));
    };
    ...
    const handleGenerateClick = async () => {
        ...
        const generationThunks = generationTasks.map((task) => (async () => {
            try {
        ...
        for (let i = 0; i < generationThunks.length; i += CONCURRENCY_LIMIT) {
            const batch = generationThunks.slice(i, i + CONCURRENCY_LIMIT);
            await Promise.all(batch.map(thunk => thunk()));
        }
    ...
    const handleDownloadRequest = async (imageUrl: string, era: string, prompt: string) => {
        // Sanitize the era name to create a valid filename using spaces.
        const fileName = `${era.toLowerCase().replace(/[^a-z0-9\s]/g, ' ').replace(/\s+/g, ' ').trim()}.png`;
        try {
            const croppedImageUrl = await cropImage(imageUrl, '1:1');
            const imageWithMetadata = embedPromptInImage(croppedImageUrl, prompt);
            await triggerDownload(imageWithMetadata, fileName);
        } catch (err) {
    ...
    const handleSaveAllClick = async () => {
        ...
        try {
            for (let i = 0; i < successfulImages.length; i++) {
                ...
                await handleDownloadRequest(img.imageUrl!, img.id, fullPrompt);
    ...
    const handleAlbumDownloadRequest = async () => {
        ...
        try {
            ...
            await triggerDownload(stitchCanvas.toDataURL('image/png'), `prompt-comparison-album.png`);
        } catch (err) {
            const errorMessage = err instanceof Error ? err.message : "An unknown error occurred.";
            ...
            <div className="min-h-screen flex flex-col items-center p-4 sm:p-8 pb-20">
                ...
                <div className="w-full max-w-6xl mx-auto">
                    <header className="text-center my-12">
                        <h1 className="text-4xl md:text-5xl font-bold text-gray-900 tracking-tighter">Batch Prompt Extraction & Comparison Tool</h1>
                               ...
                               >
                                ...
                                <div className="space-y-6">
                                 ...
                                 <div ref={promptsRef}>
                                    <Section 
                                        title="Your Prompts"
                                        subtitle="Each prompt will generate a separate image. Up to 70 prompts are supported."
                                        presetType="style"
